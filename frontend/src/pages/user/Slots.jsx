...
export default function Slots() {
  ...
  const book = (slot) => {
    ...
    // Navigate to map navigation page
    navigate("/navigate", {
      state: {
        id: slot.id,
        code: slot.code,
        ...
        <div className="min-h-screen text-white p-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            {slots.map(slot => (
              <div
                key={slot.id}
                className={`p-6 rounded-xl shadow-xl text-center font-bold transition transform hover:scale-105
              ...
              >
                <div className="text-3xl">{slot.code}</div>
