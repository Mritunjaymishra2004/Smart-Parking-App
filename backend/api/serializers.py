...
class ParkingLotSerializer(
    ...
    calculated_available_slots = serializers.IntegerField(
        read_only=True
    )
...
class SlotSerializer(
    ...
    zone_label = serializers.CharField(
        source="zone.label",
        read_only=True
    )

    lot_name = serializers.CharField(
        source="zone.lot.name",
        read_only=True
    )
...
class VehiclePositionSerializer(
    serializers.ModelSerializer
):
...
class ReservationSerializer(
    ...
    def validate(
        ...
        if start_time < timezone.now():
            ...
            raise serializers.ValidationError(
                "Cannot create reservation in past."
            )
...
class PaymentSerializer(
    ...
    def validate(
        ...
        if not attrs.get("session") and not attrs.get("reservation"):

            raise serializers.ValidationError(
                "Session or reservation required."
            )
